...
object Fixings {
	...
	def keySet:Set[String] = mapper.keySet
  	...
  	val cmtMap:Map[String, JavaDate => Option[(JavaDate, Double)]] = Map("CMT10" -> ((d:JavaDate) => DB.getLatestParam("Fixing", "JGBY", "10Y", d)))
  	
  	val currencySet = Currencies.keySet
  
	val cashMaturities = Set("1M", "2M", "3M", "6M")
	val cashMap = for (c <- currencySet; m <- cashMaturities) yield ((c + m) -> ((d:JavaDate) => DB.getLatestParam("Cash", c, m, d)))
	
    val swapMaturities = Set("1Y", "2Y", "3Y", "4Y", "5Y", "7Y", "10Y", "15Y", "20Y", "30Y")
    val swapMap = for (c <- currencySet; m <- swapMaturities) yield ((c + m) -> ((d:JavaDate) => DB.getLatestParam("Swap", c, m, d)))
    
    val fxMap = 
      currencySet.map(c => (c + "JPY") -> ((d:JavaDate) => DB.getLatestFX(c, d))) ++ 
      currencySet.map(c => ("JPY" + c) -> ((d:JavaDate) => DB.getLatestFX(c, d).collect{case (d, n) => (d, 1/n)})) ++
      (for (c1 <- (currencySet - "JPY"); c2 <- (currencySet - "JPY")) yield((c1 + c2) -> ((d:JavaDate) => DB.getLatestFX(c1, c2, d))))
