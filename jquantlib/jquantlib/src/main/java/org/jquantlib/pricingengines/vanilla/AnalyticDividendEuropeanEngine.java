...
public class AnalyticDividendEuropeanEngine extends DividendVanillaOption.EngineImpl {
    ...
    public void calculate() /* @ReadOnly */ {
        ...
        final Date settlementDate = process.riskFreeRate().referenceDate();
        double riskless = 0.0;
        for (int i=0; i<a.cashFlow.size(); i++) {
            final CashFlow cashflow = a.cashFlow.get(i);
            if (cashflow.date().gt(settlementDate)) {
                riskless += cashflow.amount() * process.riskFreeRate().discount(cashflow.date());
            }
        }

        final double spot = process.stateVariable().value() - riskless;
        QL.require(spot > 0.0, "negative or null underlying after subtracting dividends"); // TODO: message

        final /*@DiscountFactor*/ double dividendDiscount = process.dividendYield().discount(a.exercise.lastDate());
        final /*@DiscountFactor*/ double riskFreeDiscount = process.riskFreeRate().discount(a.exercise.lastDate());
        ...
        /*@Time*/ double t = voldc.yearFraction(process.blackVolatility().referenceDate(), a.exercise.lastDate());
        ...
        for (int i = 0; i < a.cashFlow.size(); i++) {
            ...
            if (d.gt(settlementDate)) {
                ...
                * process.riskFreeRate().zeroRate(d, rfdc, Compounding.Continuous, Frequency.Annual).rate()
                * process.riskFreeRate().discount(d);
                delta_rho += cashflow.amount() * process.time(d) * process.riskFreeRate().discount(t);
            }
        }
