...
//    val strikes = (callVariables, strikeList).zipped.toMap.getOptionalDecimal
    val strikes:UnderlyingFixing = UnderlyingFixing(fixedNode.collect{case n => Payoff.nodeToComputedMap(n, "strike", callVariables)}.getOrElse(Map.empty))(fixingInfo.getStrikeFixingInformation)
