...
class FXparameter (
	...
	def addFXVol(v: Double):FXparameter = new FXparameter(vol.mapValues(_ + v), rr25, rr10, bf25, bf10)
...
object FXparameter {
	...
	def getParams(params:Set[RateFXParameter]):Map[String, FXparameter] = 
	  params.groupBy(_.asset).mapValues(paramset => {
	    var fx = new FXparameter
	...
	private def createMap(params:Set[RateFXParameter]):Map[qlPeriod, Double] = params.map(p => (new qlPeriod(p.maturity), p.value)).toMap
}
