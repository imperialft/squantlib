 ...
 */
case class Microsphere3D(
  var valuedate:Date, 
  values:Map[(Double, Double), Double],
  useExtrapolation:Boolean = false
) extends YieldParameter3D {
	
  def minValue = values.values.min
  def maxValue = values.values.max
	
  def value(x1:Double, x2:Double):Double = if(useExtrapolation) interpolation(x1, x2) else math.max(minValue, math.min(maxValue, interpolation(x1, x2)))
...
//    val valarray = values.values.toArray
    val valArray = values.toArray.sortBy{case ((a, b), c) => (a, b)}
    val keysarray = valArray.map{case ((a, b), c) => Array(a, b)}
    val valarray = valArray.map{case ((a, b), c) => c}
...
object Microsphere3D{
  ...
  def apply(valuedate:Date, values: => Map[(qlPeriod, qlPeriod), Double]):Microsphere3D = apply(valuedate, values, false)

  def apply(valuedate:Date, values: => Map[(qlPeriod, qlPeriod), Double], useExtrapolation:Boolean):Microsphere3D = 
    new Microsphere3D(valuedate, values.map{case ((d1, d2), v) => ((valuedate.days(d1).toDouble, valuedate.days(d2).toDouble), v)}, useExtrapolation)
